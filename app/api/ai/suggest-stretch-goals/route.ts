...
import { StretchGoalSuggestionService } from '@/lib/services/StretchGoalSuggestionService';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const service = new StretchGoalSuggestionService();
    
    const result = await service.suggestStretchGoals(body);
    ...
    console.error('Stretch goal suggestion error:', error);
    ...
    return NextResponse.json({ 
      error: 'Failed to generate stretch goal suggestions',
      type: 'unknown_error' 
    }, { status: 500 });
  }
}